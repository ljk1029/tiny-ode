...
#elif defined(__unix__) || defined(__unix) || defined(__APPLE__) || defined(__MACH__) || defined(__linux__) || defined(__linux)
...
// 定义一个宏来简化 print 函数调用
#define prints(...) do { \
    std::cout << "[" << __FILE__ << ":" << __LINE__ << " " << __func__ << "] pid: " << GET_PID() << ", tid: " << std::this_thread::get_id() << ", "; \
    print(__VA_ARGS__); \
} while(0)
...
public:
    TestRunner() : run_in_all(false), run_cases("") {
        print("run all:", run_in_all);
        print("run case:", run_cases);
    }
    ...
    // 非void返回类型的泛型函数打印重载
    template <typename ReturnType>
    ReturnType funcPrint(const std::string &title, CallbackFunc<ReturnType> callback) {
    ...
    // 获取运行状态
    bool getRunStatus(const std::string &option) {
        return run_in_all || run_cases.find(option) != std::string::npos;
    }
