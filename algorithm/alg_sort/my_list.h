...
public:
    ...
    // 获取大小
    int getListSize() { return list_cur_size; }

    void printList(std::string title);

private:
    // 打印提示
    template <typename... Args> void printTip(const Args &...args) {
        if (log_switch) {
            std::cout << "Info: ";
            ((std::cout << args), ...);
            std::cout << std::endl;
...
// 定义在类外
template <typename T> void LinkedList<T>::printList(std::string title) {
