...
class Solution {
// 用前序遍历和中序遍历创建二叉树
TreeNode* treePreToInHelper(std::vector<int>& preorder, int preStart, int preEnd, 
    ...
    root->left  = treePreToInHelper(preorder, preStart + 1, preStart + leftTreeSize, 
                                     inorder, inStart, rootIndex - 1);
    root->right = treePreToInHelper(preorder, preStart + leftTreeSize + 1, preEnd, 
...
// 用中序遍历和后序遍历创建二叉树
TreeNode* treeInToPostHelper(std::vector<int>& inorder, int inStart,   int inEnd, 
    ...
    int leftTreeSize = rootIndex - inStart;
    root->left  = treeInToPostHelper(inorder, inStart,   rootIndex - 1, 
                                  postorder, postStart, postStart + leftTreeSize - 1);
    root->right = treeInToPostHelper(inorder, rootIndex + 1, inEnd, 
...
// 用前序遍历和后序遍历创建二叉树
TreeNode* treePreToPostHelper(std::vector<int>& preorder, int preStart,  int preEnd, 
                             std::vector<int>& postorder, int postStart, int postEnd) {
    ...
    while (postorder[postStart + leftTreeSize] != leftRootValue)
        leftTreeSize++;
    root->left = treePreToPostHelper(preorder, preStart + 1, preStart + leftTreeSize + 1, 
                                    postorder, postStart, postStart + leftTreeSize);
    root->right = treePreToPostHelper(preorder, preStart + leftTreeSize + 2, preEnd, 
