...
bool DFSUtil(int x, int y, int n, int m, const vector2 &array, vector2 &judge,
    ...
    // 向右移动
    if (DFSUtil(x + 1, y, n, m, array, judge, path))
        return true;

    // 向下移动
    if (DFSUtil(x, y + 1, n, m, array, judge, path))
        return true;
    ...
    while (!q.empty()) {
        ...
        for (auto &dir : dirs) {
            ...
            if (next_x >= 0 && next_x < n && next_y >= 0 && next_y < m &&
                array[next_x][next_y] == 1 && judge[next_x][next_y] == 0) {
...
void test_DFS_BFS() {
    ...
    std::vector<std::vector<int>> array = {{1, 1, 1, 0, 1, 1, 0},
                                           ...
                                           {0, 1, 0, 1, 1, 1, 1}};
#endif
    std::cout << std::boolalpha << "DFS find path result: " << DFS(array)
              << std::endl;
